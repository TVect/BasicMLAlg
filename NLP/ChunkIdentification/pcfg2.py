...
class PCFG(object):
    def __init__(self, non_ternimal, terminal, rules_prob, start_prob):
        ...
        # sort non_terminals to solve the dependence
        # i.e. a rule A -> B contains that B should be computed before A
        for key, item in self.rules_prob.iteritems():
            if key not in self.non_terminal:
                self.non_terminal.append(key)
            for rule in item:
                if len(rule) == 1 and rule[0] in non_ternimal:
                    self.non_terminal.insert(self.non_terminal.index(key), rule[0])
        self.best_path = None
                ...
                for x in self.non_terminal:
                    ...
                    for key, value in self.rules_prob[x].iteritems():
                        if key[0] not in self.non_terminal: 
                            break
                        if len(key) == 1:
                            tmp_best_x['prob'] = value * best_path[i][j][key[0]]['prob']
                            tmp_best_x['path'] = {'split': None, 'rule': key}
                            continue
                        for s in range(i, j):
